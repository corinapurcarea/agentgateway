...
use crate::cel::{BufferedBody, Expression, RequestSnapshot, Value};
...
impl ExtAuthzDynamicMetadata {
	/// Merge another metadata map into this one. Keys from `other` take precedence on conflict.
	pub fn merge(&mut self, other: ExtAuthzDynamicMetadata) {
...
impl ExtAuthz {
	...
	fn get_header_values(
		...
		collect_header_values(req.headers(), name, headers);
	}
	...
	pub async fn check_mcp(
		...
		let metadata_context = self.build_metadata_from_snapshot(metadata, snapshot, mcp);
	...
	fn build_metadata_from_snapshot(
		&self,
		metadata: &Option<HashMap<String, Arc<cel::Expression>>>,
		snapshot: &RequestSnapshot,
		mcp: Option<&ResourceType>,
...
fn collect_header_values(
	...
	let values: Vec<String> = header_map
		.get_all(name)
		.iter()
